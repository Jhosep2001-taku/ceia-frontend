...
const AgregarImagenSolicitud = ({ solicitudId, onImagenesAgregadas }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!solicitudId) {
            setSnackbar({ open: true, message: 'Debe seleccionar una solicitud.', severity: 'error' });
            return;
        }
        ...
        try {
            const response = await axiosInstance.post(`${API_URL}/solicitudes/${solicitudId}/imagenes`, formData, {
    ...
    return (
        <Box sx={{ mt: 4 }}>
            <Typography variant="h5" gutterBottom>Agregar Imágenes a Solicitud</Typography>
            <form onSubmit={handleSubmit}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    ...
                    <Typography variant="body2" color="textSecondary" sx={{ ml: 2 }}>
                        Seleccione una o más imágenes para la solicitud
                    </Typography>
                </Box>
...
export default AgregarImagenSolicitud;
