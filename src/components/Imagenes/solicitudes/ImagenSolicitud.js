...
import AgregarImagen from './AgregarImagenSolicitud';
import AgregarImagenConCamara from './AgregarImagenSolicitudConCamara';
import EliminarImagenSolicitud from './EliminarImagenSolicitud';
import { API_URL } from '../../../config';

const ImagenSolicitud = ({ solicitudId }) => {
    ...
    const fetchImagenes = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await axiosInstance.get(`${API_URL}/solicitudes/${solicitudId}/imagenes`);
    ...
    return (
        <Box sx={{ mt: 4 }}>
            <Typography variant="h5" gutterBottom>Imágenes de la solicitud</Typography>
            ...
            {isLoading ? (
                ...
                <Typography variant="body1">No hay imágenes para esta solicitud.</Typography>
            ) : (
                <>
...
export default ImagenSolicitud;
